...
sub getstrvar{
  $strnum++;
  my $res="$prefix\_tempstr_$strnum";
  push @strarray,$res;
  return $res;
}
...
sub concatem {
  ...
  my $seennontrivial=0;
  for my $elem (@array){
    next if($elem eq "t" and $lastelem eq "t");
    my $newelem=$elem;
    $newelem=getstrvar() if($elem eq "t");
    push @tmp,$newelem;
    $lastelem=$elem;
    if($elem ne "t"){
	 $seennontrivial=1;
  ...
  return unless $seennontrivial; 
  #print "\n\n Original : @array \n Final : @tmp \n\n";
  #print "$lhs := ".join(".",@tmp)."\n" if $seennontrivial;
  my $len=@tmp;
  if($len==1){
    push @matchrules,"$lhs == @tmp"; 
    return;
  }
  
  if($len==2){
    push @matchrules,"$lhs := ".join(' . ',@tmp); 
    return;
  }
  ...
  my $t1=shift @tmp;
  my $t2=shift @tmp;
  push @matchrules,"$str := $t1 . $t2";
  my $extra=pop @tmp;
  $t1=$str;
  for $t2 (@tmp){
    $t1=getstrvar();
    push @matchrules,"$t1 := $str . $t2";
    $str=$t1;
  }
  
  push @matchrules,"$lhs := $str . $extra"
   
}
  

sub pcre_tohampi::capture::domatch {
     my $self=shift; 
	$capturenum++;
	my $str="$prefix\_idx_$capturenum";
	my @concatarray=();
	my $res="";
	my $reg=$self->fullstring();
	for my $child (@{$self->{CONTENT}}){
	  next if($child->type eq "alt");
	  if($child->type eq "capture" or $child->type eq "group"){
	    $res=$child->domatch;
	 } else { 
	   
	   $res="t";}
	 push @concatarray,$res;
	}
     concatem($str,@concatarray);
	if($self->quant eq "*" || $self->quant eq "+"){
	  @concatarray=("t",$str);
	  $gnum++;
	  $str="$prefix\_extra_$gnum";
	  push @strarray,$str;
	  concatem($str,@concatarray);
	  push @matchrules,"$str \\in CapturedBrack(\/$reg\/, 0)";
	}
...
sub pcre_tohampi::group::domatch {
     my $self=shift; 
	$gnum++;
	my $str=$prefix.'_group_'.$gnum;
	push @strarray,$str;
	my @concatarray=();
	my $res="";
	for my $child (@{$self->{CONTENT}}){
...
sub getmatchconstraints {
	my $self=shift;
     $prefix=shift;
	
	($gnum,$capturenum,$strnum)=(0,0,0);
	@strarray=();
	@matchrules=();
	my $node=@{ $self->{TREE} }[0];
	$regex=$self->display();
	#print "Regex is : $regex \n\n";
	my $temp=$node->domatch;
	push @matchrules,"$temp == $prefix\_idx_0;";
    
	#print 'type '.join(": CSTR;\ntype ",@strarray)." : CSTR;\n";
	
	return $capturenum,@matchrules;
}
