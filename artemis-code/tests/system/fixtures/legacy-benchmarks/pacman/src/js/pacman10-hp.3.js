...
function ()
{
    ...
    g.preparePaths = function ()
    {
        ...
        for (b in o) 
        	if (o[b].w) for (h = o[b].x * 8; h <= (o[b].x + o[b].w - 1) * 8; h += 8)
        	{
    ...
    g.addEventListeners = function ()
    {
        ...
        {
            window.addEventListener("onkeydown", g.handleKeyDown, e);
            g.canvasEl.addEventListener("onclick", g.handleClick, e);
            g.registerTouch()
        }
        else
        {
    ...
    g.newLevel = function (b)
    {
        ...
        if ((g.levels.frightTime > 0) && (g.levels.frightTime <= 6)) g.levels.frightTime = Math.round(g.levels.frightTime * D);
        // end issue 14
    ...
    {
    	g.tilesChanged = e;
    	for (var b = g.playerCount; b < g.playerCount + 4; b++) for (var c = 0; c < g.playerCount; c++) if (g.actors[b].tilePos[0] == g.actors[c].tilePos[0] && g.actors[b].tilePos[1] == g.actors[c].tilePos[1]) 
