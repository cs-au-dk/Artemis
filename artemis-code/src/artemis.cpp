...
QUrl parseCmd(int argc, char* argv[], artemis::Options& options)
{
    ...
    std::string usage = "\n"
            ...
            "           article-cov - a combination of the constant and the coverage strategy, as defined in the Artemis article\n"
            "           article-all - a combination of the constant, the coverage and the readwrite strategy, as defined in the Artemis article\n"
            "\n"
    ...
    while ((c = getopt_long(argc, argv, "ehsrp:a:m:f:t:c:i:v:", long_options, &option_index)) != -1) {
        ...
        case 'z': {
            ...
            } else if (string(optarg).compare("article-cov") == 0 ){
                options.prioritizerStrategy = artemis::COVERAGE_STRATEGIES;
            } else {
