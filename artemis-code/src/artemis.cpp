...
QUrl parseCmd(int argc, char* argv[], artemis::Options& options)
{
    ...
    std::string usage = "\n"
            ...
            "\n"
            "--event-filter-area <XPath>:\n"
            "           The given XPath identifies a subtree of the DOM, and events are only chosen from this subtree.\n"
            "           In major-mode 'artemis'', the events chosen must belong to elements within this area.\n"
            "           In major-mode 'concolic' or 'concolic-reordering', the analysis will fill input fields within this area.\n"
            "           If not supplied, the entire page is considered.\n"
            "\n"
    ...
    {"event-filter-area", required_argument, NULL, 'X'},
    ...
    while ((c = getopt_long(argc, argv, "ehsrp:a:m:I:F:f:t:c:i:v:", long_options, &option_index)) != -1) {
        ...
        case 'X': {
            options.eventFilterArea = QString(optarg);
            break;
        }
