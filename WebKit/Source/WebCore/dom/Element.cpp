...
std::string Element::getXPath()
{
    ...
    // id attributes
    std::string idAttr = getIdAttribute().string().ascii().data();
    ExceptionCode ec;
    WTF::RefPtr<WebCore::NodeList> list;
    std::string selector = std::string("#") + idAttr;
    if (idAttr.length() > 0 &&
            idAttr.find("ARTEMISID-") == std::string::npos &&
            (list = WTF::getPtr(document()->querySelectorAll(WTF::String(selector.c_str()), ec))) != NULL &&
            list->length() == 1) {
        // if the element has an ID, it is not one generated by Artemis, and only one id exist
        return std::string("//") + tn + "[@id=\"" + idAttr + "\"]";
    }

    // child selection

    std::string pathFragment = tn;
    Element* p = parentElement();

    if (p->childElementCount() != 1) {
        // count number of siblings with the same tag until we reach the current element

        Element* cur = p->firstElementChild();
        unsigned counter = 1;

        while (this != cur && cur != NULL) {
            if (tn.compare(cur->tagName().lower().ascii().data()) == 0) {
                counter++;
            }
            cur = cur->nextElementSibling();
        }

        ASSERT(cur != NULL); // we should always find the cur element

        std::stringstream str;
        str << tn << "[" << counter << "]";

        pathFragment = str.str();
    }

    std::string parentPathFragment = p->getXPath();
    return parentPathFragment + (parentPathFragment.compare("/") == 0 ? "" : "/") + pathFragment;
}
